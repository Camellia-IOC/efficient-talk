        ...
        <div class="selected-list-container">
          ...
          <div class="operation-bar">
            <a-button type="primary"
                      @click="handleConfirm"
                      :disabled="groupName === ''||selectedUserList.length===0"
            >创建
            </a-button>
    ...
    const handleConfirm = () => {
        if (selectedUserList.value.length !== 0) {
            if (groupName.value !== "") {
                ...
                if (!idList.includes(curUserId.value)) {
                    idList.push(curUserId.value);
                }
                ChatApi.createChatGroup({
                    groupName: groupName.value,
                    creator: curUserId.value,
                    orgId: curOrgId.value,
                    memberList: idList
                }).then((response) => {
                    const res = response.data;
                    if (res.code === 0) {
                        ...
                        message.error("群聊创建失败");
                    }
                });
                dialogClose();
            }
            else {
                message.error("群聊名称不能为空");
            }
        }
        else {
            message.error("未选择用户");
